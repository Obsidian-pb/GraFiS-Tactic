...
Private c_ManagementTimeLine As c_ManagementTimeLine 'Переменная в которой хранитя ссылка на c_ManagementTimeLine
...
'    Dim c_ManagementTech As c_ManagementTechnics 'Переменная в которой хранитя ссылка на c_ManagementTechnics
...
'Процедура активирует форму ManagementTimeLine
    If c_ManagementTimeLine Is Nothing Then
        Set c_ManagementTimeLine = New c_ManagementTimeLine
    Else
        c_ManagementTimeLine.PS_ShowWindow
    End If
    ShpObj.Delete
End Sub
