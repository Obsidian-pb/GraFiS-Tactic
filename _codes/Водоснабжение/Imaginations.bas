...
Set ShapeFrom = Application.ThisDocument.Masters("Открытый водоисточник").Shapes(1)

IDTo = ShapeTo.ID
IDFrom = Application.ThisDocument.Masters("Открытый водоисточник").Index
...
Set ShapeFrom = Application.ThisDocument.Masters(ShapeFromID).Shapes(1) 'Application.ActivePage.Shapes.ItemFromID(ShapeFromID)
...
'---Присваиваем объектным переменным Фигуры(ShapeFrom и ShpeTo) в соответствии с индексами
Set ShapeFrom = Application.ThisDocument.Masters(ShapeFromID).Shapes(1)
Set ShapeTo = Application.ActivePage.Shapes.ItemFromID(ShapeToID)
...
'MsgBox Application.ThisDocument.Masters(ShapeFromID).Name
