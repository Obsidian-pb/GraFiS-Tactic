...
'---Вбрасываем фигуру подписи площади
    ...
    Set shpLabel = Application.ActiveWindow.Page.drop(Application.Documents.Item("Водоснабжение.vss").Masters.ItemU("Подпись названия водоисточника"), pntX, pntY)
...
'---Вбрасываем фигуру объема вобоисточника
    '---Определяем точки Х и У для вброса
        pntX = ShpObj.CellsU("pinX")
        pntY = ShpObj.CellsU("pinY")
    Set shpLabel = Application.ActiveWindow.Page.drop(Application.Documents.Item("Водоснабжение.vss").Masters.ItemU("Объем открытого водоисточника"), pntX, pntY)
...
'Процедура добавления strelki rasstoiania с фигурой водоисточника
'---Объявляем переменные
...
vsi_ShapeIndex = 0
    ...
    '---Перебираем все фигуры и nahodim ochag
