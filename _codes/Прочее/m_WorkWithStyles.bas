...
'---Обновляем стили активного документа
    ...
    '---Перебираем все стили трафарета
        ...
        '---Проверяем есть ли указаный стиль Other_StyleSet(i) в активном документе
            If StyleExist(Other_StyleSet(i)) Then
            '---Если есть - обновляем его
                StyleRefresh Other_StyleSet(i)
            Else
            '---Если нет - вбрасываем стиь Other_StyleSet(i) в активный документе
